"""PowertrainDelphiSecurityAlgo — ported from jglim/UnlockECU (MIT, (c) 2020 JinGen Lim).

Faithful Python port of ``PowertrainDelphiSecurityAlgo.cs``. Similar to the generic
PowertrainSecurityAlgo but with a fixed i/j bit-selection table, specific to the
CRD2, CRD3, CRD3S2 family.
"""
...
class PowertrainDelphiSecurityAlgo(SecurityProvider):
    NAME = "PowertrainDelphiSecurityAlgo"
    ...
    def generate_key(self, seed: bytes, key_length: int,
        ...
        y = (working_seed[1] ^ working_seed[0]) & 0xFF
        d_bit2 = get_bit((working_seed[1] ^ working_seed[0]) & 0xFF, 3)
        d_bit1 = get_bit(working_seed[2], 3)
        d_bit0 = get_bit(working_seed[3], 6)
        ...
        g_bit0 = get_bit(working_seed[1], 3)
        g_bit1 = get_bit(y, 7)
        g_bit2 = get_bit(get_byte(d_xor_intermediate, 1), 2)
