"""PowertrainSecurityAlgo3 — ported from jglim/UnlockECU.

Faithful Python port of ``Security/PowertrainSecurityAlgo3.cs`` from UnlockECU
(https://github.com/jglim/UnlockECU, MIT, (c) 2020 JinGen Lim).

CRD3 Level 1. Originally written by Aloulou, raised by Wilhelm Sorban
(@WSorban) at https://github.com/jglim/UnlockECU/issues/15. Named
PowertrainSecurityAlgo3 by convention. There are no configurable parameters.

Remaps 7 seed bits into a 0..127 index, then XORs the seed with a
seed-masked constant and a 128-entry lookup table.
"""
...
class PowertrainSecurityAlgo3(SecurityProvider):
    NAME = "PowertrainSecurityAlgo3"

    _FINAL_XOR_KEY = 0x40088C88
    _SOURCE_BIT_POSITIONS = [3, 7, 10, 11, 15, 19, 30]
    _LOOKUP_TABLE = [
    ...
    @staticmethod
    def _remap_bit(in_value: int, source_bit_position: int, destination_bit_position: int) -> int:
        return ((1 << destination_bit_position) if (in_value & (1 << source_bit_position)) > 0 else 0) & U32
    ...
    def generate_key(self, seed: bytes, key_length: int,
        ...
        # Remap 31 bits of seed into 7 bits, spanning 0..0x7F (both inclusive).
        remapped_value = 0
        for idx in range(len(self._SOURCE_BIT_POSITIONS)):
            remapped_value |= self._remap_bit(seed_val, self._SOURCE_BIT_POSITIONS[idx], idx)
        remapped_value &= U32

        key = (seed_val ^ (seed_val & self._FINAL_XOR_KEY) ^ self._LOOKUP_TABLE[remapped_value]) & U32

        out_key = bytearray(int_to_bytes(key, BIG))
